...
**完全基于 Changeset 的数据库抽象层，零 GORM 泄露，Multiple Adapters 支持，生产级别的迁移工具链**

## 📋 目标对比

| 维度 | v0.2.x (当前) | v1.0.0 (目标) |
|------|--------------|--------------|
| CRUD 接口 | 需要暴露 GORM | 完全基于 Changeset |
| 事务支持 | 有限，直接 exec | 完整的事务语义 |
| 批量操作 | 需要 GORM | Changeset 原生支持 |
| 数据库支持 | PostgreSQL, MySQL, SQLite | +SQL Server, +自定义 |
| 数据源 | 单一 SQL 数据库 | Multiple Adapters (SQL + 搜索引擎) |
| 迁移工具 | 手动编写 | 完整工具链 |
| Schema 生成 | 无 | 从数据库反向生成 |
| 版本控制 | 无 | 自动分表监控 |
...
### 1. 完全基于 Changeset 的 CRUD API

#### 1.1 核心原则
- ✅ **不暴露 GORM**：将 GORM 作为纯粹的 driver
- ✅ **事务优先**：所有写操作默认在事务中执行
- ✅ **Changeset 中心**：所有数据变更通过 Changeset 验证

#### 1.2 新增 Repository API
...
type Transaction struct {
    adapter Tx
    // 提供和 Repository 相同的 CRUD 方法
}
```

#### 1.3 QueryOption 和 TxOption
...
func WithSync(adapterNames ...string) TxOption // 同步到其他 Adapter
```
...
**实现**:
- `sqlserver_adapter.go` - 基本 CRUD
- `sqlserver_dynamic_table.go` - 动态表支持
- `sqlserver_migration.go` - 迁移支持

**关键差异**:
- TOP 语句代替 LIMIT
- IDENTITY 代替 AUTO_INCREMENT
- 事务隔离级别默认值
- 存储过程语法

#### 2.2 自定义 Adapter 文档

**文件**: `.dev-docs/CUSTOM_ADAPTER.md`
...
类似 Ecto 的设计，eit-db 允许自定义 Adapter 来支持任何后端存储：
...
func (t *TableBuilder) Timestamps() // 自动添加 created_at, updated_at
...
// GenerateSchemaFromTable 从数据库表生成 Schema 代码
...
> **架构定位**: ORM 层面的数据版本管理，基于 Migration 元数据表实现
...
// 版本元数据表 (由 migrator 自动管理)
var SchemaVersionsTable = DefineSchema("eit_schema_versions", func(s *SchemaBuilder) {
    s.Field("id", BigInt).PrimaryKey()
    s.Field("table_name", String).NotNull()        // 被监控的表名
    s.Field("partition_key", String).Nullable()     // 分表键 (如 "2026_01")
    s.Field("schema_hash", String).NotNull()        // Schema 结构哈希
    s.Field("row_count", BigInt).Default(0)         // 当前行数
    s.Field("size_bytes", BigInt).Default(0)        // 表大小
    s.Field("last_modified", Timestamp)             // 最后修改时间
...
var UserLogsSchema = DefineSchema("user_logs", func(s *SchemaBuilder) {
    ...
    s.Versioned(VersionConfig{
        PartitionBy: "month",                    // 按月分表
        PartitionField: "created_at",            // 分表依据字段
        MaxRowsPerPartition: 10000000,           // 单表最大行数
        MaxSizePerPartition: 5 * 1024 * 1024 * 1024, // 单表最大 5GB
        AutoCreateNextPartition: true,           // 自动创建下个分区
    })
})

// Repository API
func (r *Repository) EnableVersionControl(ctx context.Context, schema Schema) error {
    // 1. 在 eit_schema_versions 中注册表
    // 2. 启动后台监控
    // 3. 检测 schema 变更
}
...
### 6. Multiple Adapters 支持

> **核心能力**: 同时使用多个数据源（关系型数据库 + 搜索引擎）

#### 6.1 Multi-Adapter 架构
...
const (
    AdapterTypeSQL    AdapterType = "sql"    // 关系型数据库
    AdapterTypeSearch AdapterType = "search" // 搜索引擎
    AdapterTypeCache  AdapterType = "cache"  // 缓存 (保留接口)
)

// MultiRepository 支持多个 adapter
type MultiRepository struct {
    primary   Adapter           // 主数据源 (SQL)
    secondary map[string]Adapter // 辅助数据源
    mu        sync.RWMutex
}

// NewMultiRepository 创建多数据源 Repository
...
const (
    SyncStrategyImmediate SyncStrategy = "immediate" // 立即同步
    SyncStrategyAsync     SyncStrategy = "async"     // 异步同步
    SyncStrategyBatch     SyncStrategy = "batch"     // 批量同步
)
...
### v0条件构建器 (Eq, Gt, Lt, In, And, Or, Not 等)
- [ ] Query Builder 链式 API
- [ ] 关联查询和预加载
- [ ] 聚合查询 (Count, GroupBy, Having)预计 2 周)
- [ ] 新的 Repository CRUD API
- [ ] 完整的事务支持
- [ ] 移除所有 GORM 暴露
- [ ] Query Builder 增强

### v0.4.0 - SQL Server 支持 (预计 1 周)
- [ ] SQL Server Adapter
- [ ] 动态表支持
- [ ] 测试覆盖

### v0.5.0 - 迁移工具基础 (预计 2 周)
- [ ] Migration 文件结构
- [ ] MigrationAdapter 接口
- [ ] 基本命令行工具

### v0.6.0 - 迁移工具完善 (预计 1 周)
- [ ] Schema 代码生成
- [ ] 迁移状态管理
- [ ] 回滚机制

### v0.7.0 - 自定义 Adapter (预计 1 周)
- [ ] Adapter 注册机制
- [ ] 完整文档和示例
- [ ] MongoDB Adapter 示例

### v0.8.0 - 数据版本控制系统 (预计 2 周)
- [ ] Schema 版本元数据表设计
- [ ] 自动监控表大小和行数
- [ ] 分表策略和自动触发
- [ ] Migration 集成
...
### v0.9.0 - Multiple Adapters 支持 (预计 2 周)
- [ ] Multi-Adapter 架构设计
- [ ] SearchAdapter 接口定义
- [ ] Elasticsearch Adapter 实现
- [ ] Meilisearch Adapter 实现
- [ ] 数据同步策略（立即/异步/批量）
- [ ] Adapter 注册和发现机制

### v0.10.0 - 性能优化和监控 (预计 1 周)
- [ ] 日志追踪系统
- [ ] 性能监控指标
- [ ] 连接池优化
...
### 3. **Multiple Adapters 支持** (重要度: ⭐⭐⭐⭐⭐)
```go
// 同时使用 PostgreSQL + Elasticsearch
...
- **v0.3.0 - CRUD 重构**: 2 周 (最核心)
- **v0.4.0 - SQL Server**: 1 周
- **v0.5.0-0.6.0 - 迁移工具**: 3 周
- **v0.7.0 - 自定义 Adapter**: 1 周
- **v0.8.0 - 数据版本控制**: 2 周 (重点)
- **v0.9.0 - Multiple Adapters**: 2 周 (重点)
- **v0.10.0-0.11.0 - 完善**: 2 周
- **v1.0.0 - 发布**: 1 周
...
1. **零 GORM 泄露**: 应用层完全不需要导入 GORM
2. **完整事务**: 所有写操作支持事务和回滚
3. **多数据库**: PostgreSQL, MySQL, SQLite, SQL Server 全支持
4. **迁移工具**: 从创建到执行到回滚的完整工具链
5. **代码生成**: 从数据库自动生成 Schema 代码
6. **版本控制**: 基于 Migration 的自动分表监控系统
7. **Multiple Adapters**: 同时使用关系型数据库和搜索引擎
8. **搜索引擎**: Elasticsearch、Meilisearch 原生支持
9. **生产就绪**: 日志、监控、错误处理完善
10. **测试覆盖**: 90%+ 代码覆盖率
11. **文档完整**: API 文档、使用指南、Multi-Adapter 最佳实践
...
│  - 缓存策略                                         │
├────────────────────────────────────────────────────┤
│              eit-db (ORM + Data Layer)             │
│  - 数据持久化 (PostgreSQL/MySQL/SQLite/SQL Server) │
│  - Schema 定义和验证 (Changeset)                   │
│  - 事务管理                                         │
│  - 数据版本控制和自动分表                           │
│  - Multiple Adapters 支持                          │
...
func (s *ArticleService) PublishArticle(ctx context.Context, id int64) error {
    // 1. 从数据库获取（eit-db - PostgreSQL）
    article, err := s.repo.Get(ctx, ArticleSchema, id)
    if err != nil {
        return err
    }
    
    // 2. 更新状态（应用层逻辑，eit-cms 负责）
    ...
    // 3. 保存并同步到搜索引擎（eit-db 的 Multi-Adapter 能力）
...
### eit-db 不负责的内容（应用层关注点）

- ❌ HTTP 缓存（由 eit-cache 处理）
- ❌ 会话管理（由 eit-cache 处理）
- ❌ 业务逻辑验证（由 eit-cms 处理）
- ❌ API 限流（由 eit-cms 处理）
- ❌ 软删除逻辑（应用层实现，只需在 schema 中定义 deleted_at 字段）
- ❌ 内容状态工作流（应用层建表时定义 status 字段）
- ❌ 多语言内容结构（应用层建表考虑，orm 只负责持久化）
- ❌ 媒体文件处理（需要独立的文件管理库）
- ❌ 多租户逻辑（MySQL/SQLite 在应用层实现，PG/SQL Server 使用专门方案）

### eit-db 专注的内容（ORM 架构层面）

- ✅ 数据库连接和查询（多数据库支持）
- ✅ Schema 定义和验证（Changeset）
- ✅ 事务管理和回滚
- ✅ 数据库迁移工具链
- ✅ 数据版本控制和自动分表监控（基于 Migration 元数据）
- ✅ Multiple Adapters 支持（同时使用 SQL + 搜索引擎）
- ✅ 搜索引擎集成（Elasticsearch、Meilisearch）
- ✅ 数据同步策略（立即/异步/批量）
- ✅ 动态表创建和管理
- ✅ 定时数据库任务（如按月建表）
...
// ✅ GORM 完全隐藏在 adapter 内部
type gormAdapter struct {
    db *gorm.DB // 私有，外部不可访问
}
...
**策略**:
1. **默认事务**: 所有写操作自动包装在事务中
2. **显式事务**: 支持手动管理事务边界
3. **嵌套事务**: 使用 Savepoint 实现（PostgreSQL）

```go
// 自动事务（推荐）
err := repo.Insert(ctx, cs) // 自动包装事务
...
**问题**: MySQL 默认不支持单次执行多条语句

**解决方案**:
1. **客户端拼接**: 在事务中多次 Exec
2. **Batch 模式**: 使用 prepared statement + executemany
3. **存储过程**: 对于复杂逻辑，使用存储过程
...
### ADR-001: 为什么不在 ORM 层实现软删除？

**决策**: 软删除属于应用层关注点，不应在 ORM 层实现。

**理由**:
1. ORM 本质是数据持久化层，不应强制业务逻辑
2. 软删除策略因业务而异（deleted_at vs is_deleted vs status）
3. 强制实现会增加一般用户的使用负担
4. Ecto 也不在 ORM 层实现软删除

**实现方式**:
- eit-cms 应用层自行实现软删除逻辑
- eit-db 只需支持在 Schema 中定义相关字段

### ADR-002: Multiple Adapters 架构

**决策**: 支持同时使用多个数据源（SQL + 搜索引擎）。

**理由**:
1. 现代应用常需要关系型数据库 + 搜索引擎组合
2. 数据同步是常见需求
3. 不同数据源适合不同场景（PostgreSQL 存储，Elasticsearch 搜索）
4. 标签和分类可以通过搜索引擎的聚合功能实现

**实现方式**:
- 主 Adapter（SQL）+ 辅助 Adapters（搜索引擎）
- 写入时可选择同步到辅助 Adapters
- 查询时可指定使用哪个 Adapter

### ADR-003: 数据版本控制基于 Migration

**决策**: 使用 Migration 元数据表实现版本控制和自动分表。

**理由**:
1. Migration 本身就是 Schema 变更的记录点
2. 可以自动监控表的增长和结构变化
3. 结合现有的动态表功能，实现自动分表
4. 不侵入应用层代码

**实现方式**:
- `eit_schema_versions` 元数据表
- 监控行数、大小、Schema 哈希
- 达到阈值时自动触发分表

### ADR-004: 搜索引擎作为 Adapter

**决策**: 将 Elasticsearch、Meilisearch 作为 Adapter 实现。

**理由**:
1. 搜索引擎本质上也是数据存储
2. 统一的接口便于切换和测试
3. 可以复用现有的 Changeset 和事务机制
4. 支持数据自动同步

**不包含的功能**:
- 标签和分类系统（应用层建表实现，或使用搜索引擎聚合）
- 复杂的聚合分析（直接使用搜索引擎客户端）

### ADR-005: 多租户不在 ORM 层实现

**决策**: 多租户架构由应用层或专门库处理。

**理由**:
1. MySQL/SQLite：缺乏数据库级隔离，必须在应用层实现
2. PostgreSQL：可使用 Citus 扩展，需要专门配置
3. SQL Server：有内置分布式支持，但配置复杂
4. 不同方案差异太大，ORM 层统一处理会过于复杂

**建议方案**:
- 轻量级场景：应用层加 tenant_id 字段
- 企业级场景：使用数据库原生方案或专门的多租户库

### ADR-006: 媒体文件管理独立

**决策**: 媒体文件处理不在 eit-db 中实现，需要独立库。

**理由**:
1. 文件管理与 ORM 关注点不同
2. 涉及文件上传、转码、CDN 等复杂逻辑
3. 可能需要对接云存储服务
4. 独立库更灵活，可单独演进

**协作方式**:
- 文件库和 eit-db 都连接同一数据库
- 文件库管理文件，eit-db 管理元数据表
- 应用层协调两者的使用

### ADR-007: 标签和分类通过搜索引擎实现

**决策**: 标签和全文搜索通过搜索引擎 Adapter 实现，而非在 ORM 层构建。

**理由**:
1. 现代搜索引擎（ES/Meilisearch）提供强大的聚合和过滤功能
2. 标签本质上是搜索的一个维度
3. ORM 层构建标签系统过于复杂
4. 搜索引擎的性能远超 SQL 的 LIKE 查询

**实现方式**:
- 应用层建立标签表（简单的 id, name, slug）
- 文章数据同步到 Elasticsearch 时包含标签信息
- 使用 ES 的 terms 聚合和过滤实现标签查询
- 分类系统类似，使用 nested 或 parent-child 关系
